-- Tokens created by the original guest Meal Plan migration encoded base64
-- padding as a trailing period. Messages preserved the period visually, but
-- the public site route stripped it before the server-side lookup. Keep
-- already-sent links valid while issuing standard unpadded base64url tokens.
