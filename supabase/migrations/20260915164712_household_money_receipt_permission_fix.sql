-- Keep updated_at system-managed. The authenticated role deliberately has no
-- direct grant for that column; the existing before-update trigger sets it to
-- the same transaction timestamp returned in each receipt.
