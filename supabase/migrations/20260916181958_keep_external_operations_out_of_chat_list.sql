-- External MCP operations need durable run/proposal/receipt records, but those
-- records are operational history rather than conversations created in Kwilt.
-- Keep them addressable for handoffs without placing them in the user's Chat list.

alter table public.kwilt_agent_threads
  add column if not exists visible_in_chat boolean not null default true;

comment on column public.kwilt_agent_threads.visible_in_chat is
  'Whether this durable thread belongs in user-facing Chat navigation.';

update public.kwilt_agent_threads as thread
set visible_in_chat = false
where exists (
  select 1
  from public.kwilt_agent_runs as run
  where run.thread_id = thread.id
    and run.origin_channel = 'external'
)
and not exists (
  select 1
  from public.kwilt_agent_runs as other_run
  where other_run.thread_id = thread.id
    and other_run.origin_channel <> 'external'
);
...
begin
  ...
  if p_origin_channel = 'external' and p_thread_id is null then
    update public.kwilt_agent_threads
    set visible_in_chat = false
    where id = (v_result ->> 'threadId')::uuid
