-- Add one signed household downvote as an alternative to positive support.
-- Positive support and downvotes remain separate signals; neither changes the
-- Plan lifecycle or blocks grocery commitment.
...
alter table public.kwilt_meal_candidate_reactions
  add constraint kwilt_meal_candidate_reactions_reaction_check
    check (reaction in ('thumbs_up','heart','yum','excited','fire','downvote'));

create or replace function public.set_kwilt_shared_meal_reaction(p_candidate_id uuid,p_reaction text)
...
begin
  ...
  if p_reaction is not null and p_reaction not in ('thumbs_up','heart','yum','excited','fire','downvote') then raise exception 'invalid_meal_reaction'; end if;
      ...
      from (
        select candidate.created_at,
          ...
          jsonb_build_object(
            ...
            'voteCount',reaction_data.vote_count,'downvoteCount',reaction_data.downvote_count,'reactionCounts',reaction_data.reaction_counts,
        ...
        cross join lateral (
          select count(*) filter(where reaction.reaction <> 'downvote')::integer vote_count,
            count(*) filter(where reaction.reaction = 'downvote')::integer downvote_count,
