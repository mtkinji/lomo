...
export type DisconnectMoneyConnectionDependencies = {
  loadOwnedConnection(
    userId: string,
    connectionId: string,
  ): Promise<DisconnectConnectionRecord | null>;
  reserve(connectionId: string, expectedUpdatedAt: string): Promise<boolean>;
...
export async function disconnectMoneyConnection(input: {
  ...
  if (
    !connectionId || !expectedUpdatedAt ||
    !Number.isFinite(Date.parse(expectedUpdatedAt))
  ) {
    throw new DisconnectMoneyConnectionError(
      "invalid_request",
      "Choose one current connection.",
      400,
    );
  }
  ...
  if (Date.parse(connection.updatedAt) !== Date.parse(expectedUpdatedAt)) {
    throw new DisconnectMoneyConnectionError(
      "connection_stale",
      "That connection changed. Refresh before continuing.",
      409,
    );
  }
  if (!await dependencies.reserve(connectionId, expectedUpdatedAt)) {
    throw new DisconnectMoneyConnectionError(
      "connection_busy",
      "This connection is being checked. Try again shortly.",
      409,
    );
  }
  try {
    ...
    if (!accessToken) {
      throw new DisconnectMoneyConnectionError(
        ...
        "The provider connection cannot be verified.",
        409,
      );
    }
    ...
    await dependencies.release(connectionId);
  }
}
