...
import { applyRemoteBankCommand, type ServerBankGame } from '../_shared/games-bank.ts';
...
Deno.serve(async (request) => {
  ...
  if (!body.sessionId || !body.participantId || !['roll', 'bank'].includes(body.actionType) || !Number.isInteger(body.expectedStateVersion) || !body.idempotencyKey) {
  ...
  const [{ data: session, error: sessionError }, { data: participant, error: participantError }] = await Promise.all([
    admin.from('game_sessions').select('id,state,state_version,status,expires_at').eq('id', body.sessionId).maybeSingle(),
  ...
  if (!session || !participant || participant.session_id !== body.sessionId || session.status !== 'active' || new Date(session.expires_at).getTime() <= Date.now()) {
    return json({ error: 'room_unavailable' }, 404);
  }
  if (participant.controller_user_id !== userData.user.id || !['local', 'joined', 'disconnected'].includes(participant.join_status)) {
    return json({ error: 'seat_not_controlled' }, 403);
  }
  if (session.state_version !== body.expectedStateVersion) {
    return json({ error: 'state_conflict', state: session.state, stateVersion: session.state_version }, 409);
  }

  let nextState: ServerBankGame;
  try {
    nextState = applyRemoteBankCommand(session.state as ServerBankGame, {
      actionType: body.actionType,
      playerId: participant.seat_index + 1,
    }, () => [secureDie(), secureDie()]);
  } catch (error) {
  ...
  const { data, error } = await admin.rpc('commit_remote_bank_command', {
    ...
    p_payload: body.actionType === 'roll' ? { dice: nextState.lastRoll } : {},
