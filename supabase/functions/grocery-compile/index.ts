...
serve(async(req)=>{
  ...
  try{
    ...
    if(planAction){
      ...
      const {data:candidateRows,error:candidateError}=await admin.from('kwilt_meal_plan_candidates').select('id,lifecycle_state,removed_grocery_behavior,recipe_snapshot').eq('plan_id',planId); if(candidateError)throw candidateError;
      ...
      const {data:receipt,error}=await admin.rpc('sync_kwilt_household_plan_groceries',{p_actor_person_id:binding.person_id,p_plan_id:planId,p_expected_version:expectedVersion,p_action:planAction,p_candidate_ids:candidateIds,p_payload_hash:payloadHash,p_compiled_items:compiled.items}); if(error)throw error;
      return json(200,{receipt});
    }
