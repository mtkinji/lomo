...
export function buildKrogerCartPayload(items: Array<{upc:string;quantity:number}>) { if(!items.length) throw new RetailerAdapterError('provider.cart_empty'); const quantities=new Map<string,number>(); for(const{upc,quantity}of items){const clean=upc.trim(),count=Math.floor(quantity);if(!clean||!Number.isFinite(count)||count<1)throw new RetailerAdapterError('provider.cart_item_invalid');quantities.set(clean,(quantities.get(clean)??0)+count);} return {items:[...quantities].map(([upc,quantity])=>({upc,quantity,modality:'PICKUP'}))}; }
