...
export function parseKrogerLocations(value: unknown): KrogerLocation[] { const data = asRecord(value).data; return (Array.isArray(data) ? data : []).flatMap((raw) => { const row=asRecord(raw), address=asRecord(row.address), id=string(row.locationId), name=string(row.name), chain=string(row.chain); if(!id||!name)return []; const locality=[string(address.city), string(address.state), string(address.zipCode)].filter(Boolean).join(' '); return [{ id, name, banner: /smith/i.test(`${chain} ${name}`) ? "Smith's" : name, address: [string(address.addressLine1), locality].filter(Boolean).join(' · ') }]; }); }
