...
const ACTIVITY_CAPTURE_SCHEMA = {
  type: 'object', properties: ACTIVITY_FIELD_PROPERTIES, required: ['title'], additionalProperties: false,
} as const;
...
export const SERVER_AGENT_TOOL_CATALOG: readonly ServerAgentToolDefinition[] = [
  {
    ...
    confirmation: 'none', canDeferToClient: false, inputSchema: EMPTY_SCHEMA, outputSchema: EMPTY_SCHEMA,
  },
  {
    ...
    confirmation: 'none', canDeferToClient: false, inputSchema: RELATIONSHIP_MEMORY_SCHEMA, outputSchema: EMPTY_SCHEMA,
  },
  {
    ...
    providers: ['server'], effect: 'write', consequence: 'low', reversible: true,
    confirmation: 'none', canDeferToClient: false, inputSchema: RELATIONSHIP_CORRECT_SCHEMA, outputSchema: EMPTY_SCHEMA,
  },
  {
    ...
    providers: ['server'], effect: 'write', consequence: 'low', reversible: true,
    confirmation: 'none', canDeferToClient: false, inputSchema: RELATIONSHIP_FORGET_SCHEMA, outputSchema: EMPTY_SCHEMA,
  },
  {
    id: 'profile.read', version: 1, capabilityId: 'profile',
    purpose: 'Read the bounded display name and age range in the current user coaching profile.', providers: ['server'],
    effect: 'read', consequence: 'low', reversible: true, confirmation: 'none',
    canDeferToClient: false, inputSchema: EMPTY_SCHEMA, outputSchema: EMPTY_SCHEMA,
  },
  {
    id: 'profile.update', version: 1, capabilityId: 'profile',
    purpose: 'Update an explicitly requested display name or age range after review.', providers: ['server'],
    effect: 'write', consequence: 'low', reversible: true, confirmation: 'explicit', canDeferToClient: false,
    inputSchema: {
  ...
  },
  {
    ...
    purpose: 'Read bounded identity Arcs owned by the current user.', providers: ['server'],
    effect: 'read', consequence: 'low', reversible: true, confirmation: 'none',
    canDeferToClient: false, inputSchema: EMPTY_SCHEMA, outputSchema: EMPTY_SCHEMA,
  },
  {
    id: 'arcs.create', version: 1, capabilityId: 'arcs',
    purpose: 'Create one explicitly reviewed identity Arc.', providers: ['server'],
    effect: 'write', consequence: 'consequential', reversible: true, confirmation: 'explicit', canDeferToClient: false,
    inputSchema: {
  ...
  },
  {
    ...
    purpose: 'Update explicit fields on one identified Arc.', providers: ['server'],
    effect: 'write', consequence: 'consequential', reversible: true, confirmation: 'explicit', canDeferToClient: false,
    inputSchema: {
      type: 'object', properties: {
  ...
  {
    id: 'goals.read', version: 1, capabilityId: 'goals',
    purpose: 'Read bounded Goals owned by the current user.', providers: ['server'],
    effect: 'read', consequence: 'low', reversible: true, confirmation: 'none',
    canDeferToClient: false, inputSchema: EMPTY_SCHEMA, outputSchema: EMPTY_SCHEMA,
  },
  {
    id: 'goals.update', version: 1, capabilityId: 'goals',
    purpose: 'Update explicit fields on one identified Goal.',
    providers: ['server'], effect: 'write', consequence: 'low', reversible: true,
    confirmation: 'explicit', canDeferToClient: false,
    inputSchema: {
      type: 'object', properties: {
  ...
  },
  {
    ...
    purpose: 'Create one explicit Goal draft, optionally linked to an existing Arc.', providers: ['server'],
    effect: 'write', consequence: 'consequential', reversible: true, confirmation: 'explicit', canDeferToClient: false,
    inputSchema: {
  ...
  },
  {
    ...
    purpose: 'Delete one identified Goal and its dependent Activities after explicit review.', providers: ['server'],
    effect: 'write', consequence: 'consequential', reversible: true, confirmation: 'explicit', canDeferToClient: false,
    inputSchema: {
  ...
  {
    id: 'activities.read', version: 1, capabilityId: 'todos',
    purpose: 'Read bounded Activities and stable step metadata owned by the current user.', providers: ['server'],
    effect: 'read', consequence: 'low', reversible: true, confirmation: 'none',
    canDeferToClient: false, inputSchema: EMPTY_SCHEMA, outputSchema: EMPTY_SCHEMA,
  },
  {
    id: 'activities.capture', version: 1, capabilityId: 'todos',
    purpose: 'Capture one explicit low-risk Activity.', providers: ['server'],
    effect: 'write', consequence: 'low', reversible: true, confirmation: 'none',
    canDeferToClient: false, inputSchema: ACTIVITY_CAPTURE_SCHEMA, outputSchema: EMPTY_SCHEMA,
  },
  {
    id: 'activities.update', version: 1, capabilityId: 'todos',
    purpose: 'Update fields on one identified Activity.', providers: ['server'],
    effect: 'write', consequence: 'low', reversible: true, confirmation: 'explicit',
    canDeferToClient: false, inputSchema: ACTIVITY_UPDATE_SCHEMA, outputSchema: EMPTY_SCHEMA,
  },
  {
    id: 'activities.delete', version: 1, capabilityId: 'todos',
    purpose: 'Delete one identified Activity after explicit review.', providers: ['server'],
    effect: 'write', consequence: 'consequential', reversible: true, confirmation: 'explicit', canDeferToClient: false,
    inputSchema: {
  ...
  },
  {
    ...
    purpose: 'Add one stable step to an identified Activity.', providers: ['server'],
    effect: 'write', consequence: 'low', reversible: true, confirmation: 'explicit', canDeferToClient: false,
  ...
  },
  {
    ...
    purpose: 'Update the title or optional state of one stable Activity step.', providers: ['server'],
    effect: 'write', consequence: 'low', reversible: true, confirmation: 'explicit', canDeferToClient: false,
  ...
  },
  {
    ...
    purpose: 'Mark one stable Activity step complete or incomplete.', providers: ['server'],
    effect: 'write', consequence: 'low', reversible: true, confirmation: 'explicit', canDeferToClient: false,
  ...
  },
  {
    ...
    purpose: 'Delete one stable Activity step.', providers: ['server'],
    effect: 'write', consequence: 'low', reversible: true, confirmation: 'explicit', canDeferToClient: false,
  ...
  },
  {
    ...
    purpose: 'Reorder stable Activity step ids; omitted steps retain their relative order at the end.', providers: ['server'],
    effect: 'write', consequence: 'low', reversible: true, confirmation: 'explicit', canDeferToClient: false,
  ...
  },
  {
    ...
    purpose: 'Set or clear the recurrence rule for one identified Activity.', providers: ['server'],
    effect: 'write', consequence: 'low', reversible: true, confirmation: 'explicit', canDeferToClient: false,
    inputSchema: {
      type: 'object', properties: {
        activityId: { type: 'string', minLength: 1 },
  ...
  },
  {
    ...
    purpose: 'Set or clear the reminder time for one identified Activity.', providers: ['server'],
    effect: 'write', consequence: 'low', reversible: true, confirmation: 'explicit', canDeferToClient: false,
    inputSchema: {
      type: 'object', properties: {
        activityId: { type: 'string', minLength: 1 },
  ...
  },
  {
    ...
    providers: ['device'], effect: 'write', consequence: 'low', reversible: true,
    confirmation: 'explicit', canDeferToClient: true, inputSchema: EMPTY_SCHEMA, outputSchema: EMPTY_SCHEMA,
  },
  {
  ...
  {
    id: 'chapters.read', version: 1, capabilityId: 'chapters',
    purpose: 'Read bounded Chapter narratives and the current user private note.', providers: ['server'],
    effect: 'read', consequence: 'low', reversible: true, confirmation: 'none',
    canDeferToClient: false, inputSchema: EMPTY_SCHEMA, outputSchema: EMPTY_SCHEMA,
  },
  {
  ...
  },
  {
    ...
    inputSchema: PLAN_DAY_SCHEMA, outputSchema: EMPTY_SCHEMA,
  },
  {
  ...
  {
    id: 'plan.schedule_activity', version: 1, capabilityId: 'plan',
    purpose: 'Schedule one identified Activity in the authoritative Plan calendar.',
    providers: ['server'], effect: 'write', consequence: 'low', reversible: true,
    confirmation: 'explicit', canDeferToClient: false,
    inputSchema: PLAN_PLACEMENT_SCHEMA, outputSchema: EMPTY_SCHEMA,
  },
  {
  ...
  },
  {
    ...
    purpose: 'Open subscription management without claiming billing changed.', providers: ['device'],
    effect: 'write', consequence: 'consequential', reversible: true, confirmation: 'explicit',
    canDeferToClient: true, inputSchema: EMPTY_SCHEMA, outputSchema: EMPTY_SCHEMA,
  },
  {
