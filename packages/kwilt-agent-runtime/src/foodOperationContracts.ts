...
export const FOOD_OPERATION_CONTRACTS: readonly FoodOperationContract[] = [
  ...
  makeContract({ id: 'meal_planning.plan.create', owner: 'meal_planning', purpose: 'Create one reviewed flexible-horizon Meal Plan for the current organizer.', authority: 'reviewed', effect: 'write', reversible: true, providers: ['device', 'server'], inputSchema: objectSchema({ horizon: { type: 'object' }, idempotencyKey: STRING_ID }, ['horizon', 'idempotencyKey']), outputSchema: RESULT, sourceRefs: ['capability:meal-planning', 'domain:mealPlanContracts'] }),
  makeContract({ id: 'meal_planning.plan.update', owner: 'meal_planning', purpose: 'Update one draft Meal Plan through optimistic reviewed version authority.', authority: 'reviewed', effect: 'write', reversible: true, providers: ['device', 'server'], inputSchema: targetSchema('mealPlanId', { expectedVersion: VERSION, patch: { type: 'object' } }, ['expectedVersion', 'patch']), outputSchema: RESULT, sourceRefs: ['domain:mealPlanContracts'] }),
  makeContract({ id: 'meal_planning.candidate.add', owner: 'meal_planning', purpose: 'Add one reviewed Recipe snapshot or meal note candidate to a draft Meal Plan.', authority: 'reviewed', effect: 'write', reversible: true, providers: ['device', 'server'], inputSchema: targetSchema('mealPlanId', { expectedVersion: VERSION, candidate: { type: 'object' } }, ['expectedVersion', 'candidate']), outputSchema: RESULT, sourceRefs: ['domain:mealPlanContracts'] }),
  ...
  makeContract({ id: 'meal_planning.plan.finalize', owner: 'meal_planning', purpose: 'Finalize one reviewed Meal Plan version under organizer authority.', authority: 'reviewed', effect: 'write', reversible: true, providers: ['device', 'server'], inputSchema: targetSchema('mealPlanId', { expectedVersion: VERSION, selectedEntries: { type: 'array', minItems: 1, maxItems: 60, items: { type: 'object' } } }, ['expectedVersion', 'selectedEntries']), outputSchema: RESULT, sourceRefs: ['domain:mealPlanContracts'] }),
  ...
  makeContract({ id: 'meal_planning.candidates.prepare', owner: 'meal_planning', purpose: 'Prepare up to three budget, stock, gap, cadence, and household-aware Meal candidates without mutating a plan.', authority: 'direct', effect: 'read', reversible: true, providers: ['device','server'], inputSchema: objectSchema({ horizon:{type:'object'}, constraints:{type:'object'} },['horizon']), outputSchema: RESULT, sourceRefs:['domain:mealCandidatePreparation'] }),
  ...
  makeContract({ id: 'food_stock.observe', owner: 'groceries', purpose: 'Prepare or record one reviewed owner-only stock observation without promoting receipt evidence to confirmed stock.', authority:'reviewed', effect:'write', reversible:true, providers:['device','server'], inputSchema:objectSchema({observation:{type:'object'},expectedObservationId:{type:['string','null']}},['observation']), outputSchema:RESULT, sourceRefs:['domain:foodStockContracts'] }),
  ...
  makeContract({ id:'store_opportunity.capture',owner:'groceries',purpose:'Capture user-supplied sale evidence as an immutable Store Opportunity without changing meals, stock, or groceries.',authority:'reviewed',effect:'write',reversible:true,providers:['device','server'],inputSchema:objectSchema({opportunity:{type:'object'}},['opportunity']),outputSchema:RESULT,sourceRefs:['domain:foodScenarioContracts'] }),
  makeContract({ id:'food_scenario.prepare',owner:'groceries',purpose:'Prepare at most three version-bound household scenarios from current plan, grocery, stock, budget, price, waste, and burden evidence.',authority:'direct',effect:'read',reversible:true,providers:['device','server'],inputSchema:objectSchema({baseline:{type:'object'},opportunityIds:{type:'array',maxItems:20,items:STRING_ID}},['baseline']),outputSchema:RESULT,sourceRefs:['domain:foodScenarioContracts'] }),
  ...
  makeContract({ id:'cook_session.start',owner:'recipes',purpose:'Start one reviewed exact-version Cook Session after readiness and serving scale are confirmed.',authority:'reviewed',effect:'write',reversible:true,providers:['device','server'],inputSchema:targetSchema('recipeVersionId',{servingScale:{type:'number',exclusiveMinimum:0}},['servingScale']),outputSchema:RESULT,sourceRefs:['domain:recipeCookContracts'] }),
  makeContract({ id:'cook_session.control',owner:'recipes',purpose:'Apply deterministic next, back, repeat, pause, resume, or timer control to the current Cook Session.',authority:'direct',effect:'write',reversible:true,providers:['device','server'],inputSchema:targetSchema('sessionId',{expectedRevision:VERSION,command:{type:'object'}},['expectedRevision','command']),outputSchema:RESULT,sourceRefs:['domain:recipeCookContracts'] }),
