...
const COOK_SESSION_COMMAND = {
  type: 'object',
  properties: {
    type: { type: 'string', enum: ['next', 'back', 'repeat', 'pause', 'resume', 'start_timer', 'pause_timer', 'resume_timer', 'cancel_timer'] },
    cueId: STRING_ID, timerId: STRING_ID,
    durationSeconds: { type: 'integer', minimum: 1, maximum: 86_400 },
    label: { type: 'string', minLength: 1, maxLength: 160 },
  },
  required: ['type'], additionalProperties: false,
} as const;
const MEAL_PLAN_HORIZON = {
  type: 'object',
  properties: {
    kind: { type: 'string', enum: ['open', 'next_shop', 'meal_count', 'date_range'] },
    shopBy: { type: ['string', 'null'], maxLength: 40 },
    count: { type: 'integer', minimum: 1, maximum: 60 },
    startsOn: { type: 'string', minLength: 10, maxLength: 40 },
    endsOn: { type: 'string', minLength: 10, maxLength: 40 },
  },
  required: ['kind'], additionalProperties: false,
} as const;
const MEAL_CANDIDATE_INPUT = objectSchema({
  candidateId: STRING_ID, title: { type: 'string', minLength: 1, maxLength: 160 },
  recipeVersionId: { type: ['string', 'null'], maxLength: 160 },
  plannedPortions: { type: 'integer', minimum: 1, maximum: 24 },
}, ['candidateId', 'title', 'recipeVersionId', 'plannedPortions']);
const MEAL_TIMING_INPUT = {
  type: 'object',
  properties: {
    kind: { type: 'string', enum: ['flexible', 'occasion', 'coverage'] },
    date: { type: 'string', minLength: 10, maxLength: 10 },
    dates: { type: 'array', minItems: 1, maxItems: 31, items: { type: 'string', minLength: 10, maxLength: 10 } },
    mealPeriod: { type: 'string', enum: ['breakfast', 'lunch', 'dinner', 'snack'] },
    label: { type: 'string', minLength: 1, maxLength: 120 },
  },
  required: ['kind'], additionalProperties: false,
} as const;
const MEAL_PLAN_DISH_INPUT = objectSchema({
  id: STRING_ID, candidateId: STRING_ID,
  dinerPersonIds: { type: 'array', minItems: 1, maxItems: 24, items: STRING_ID },
  servings: { type: ['number', 'null'], exclusiveMinimum: 0 },
}, ['id', 'candidateId', 'dinerPersonIds', 'servings']);
const MEAL_PLAN_OCCASION_INPUT = objectSchema({
  id: STRING_ID, title: { type: ['string', 'null'], maxLength: 160 },
  placementDate: { type: ['string', 'null'], maxLength: 10 }, timing: MEAL_TIMING_INPUT,
  notEatingPersonIds: { type: 'array', maxItems: 24, items: STRING_ID },
  dishes: { type: 'array', minItems: 1, maxItems: 20, items: MEAL_PLAN_DISH_INPUT },
}, ['id', 'title', 'placementDate', 'timing', 'notEatingPersonIds', 'dishes']);
const MEAL_CANDIDATE_CONSTRAINTS = objectSchema({
  query: { type: 'string', enum: ['make_now', 'almost_there', 'use_soon', 'stay_near_target', 'best_use'] },
  maxResults: { type: 'integer', minimum: 1, maximum: 3 },
});
const FOOD_STOCK_OBSERVATION_INPUT = objectSchema({
  concept: { type: 'string', minLength: 1, maxLength: 160 },
  state: { type: 'string', enum: ['confirmed', 'likely', 'check_first'] },
  quantityMin: { type: ['number', 'null'], minimum: 0 },
  quantityMax: { type: ['number', 'null'], minimum: 0 },
  unit: { type: ['string', 'null'], maxLength: 80 },
  source: { type: 'string', enum: ['already_have', 'manual', 'voice', 'photo', 'receipt', 'order'] },
  confidence: { type: 'number', minimum: 0, maximum: 1 },
  observedAt: { type: 'string', format: 'date-time' },
  expiresAt: { type: ['string', 'null'], format: 'date-time' },
}, ['concept', 'state', 'quantityMin', 'quantityMax', 'unit', 'source', 'confidence', 'observedAt', 'expiresAt']);
const GROCERY_ITEM_PATCH = objectSchema({
  concept: { type: 'string', minLength: 1, maxLength: 320 },
  quantityMin: { type: ['number', 'null'], minimum: 0 },
  quantityMax: { type: ['number', 'null'], minimum: 0 },
  unit: { type: ['string', 'null'], maxLength: 80 },
  note: { type: ['string', 'null'], maxLength: 2_000 },
});
...
export const FOOD_OPERATION_CONTRACTS: readonly FoodOperationContract[] = [
  ...
  makeContract({ id: 'recipes.import.approve', owner: 'recipes', purpose: 'Approve one reviewed Recipe import draft idempotently into canonical private content.', authority: 'reviewed', effect: 'write', reversible: true, providers: ['device', 'server'], inputSchema: targetSchema('draftId', { expectedDraftVersion: VERSION, idempotencyKey: STRING_ID, reviewedVersion: RECIPE_CHAT_DRAFT }, ['expectedDraftVersion', 'idempotencyKey', 'reviewedVersion']), outputSchema: RESULT, sourceRefs: ['domain:recipeImportContracts'] }),
  ...
  makeContract({ id: 'meal_planning.plan.create', owner: 'meal_planning', purpose: 'Create one reviewed flexible-horizon Meal Plan for the current organizer.', authority: 'reviewed', effect: 'write', reversible: true, providers: ['device', 'server'], inputSchema: objectSchema({ householdId: { type: ['string', 'null'], maxLength: 160 }, horizon: MEAL_PLAN_HORIZON, idempotencyKey: STRING_ID }, ['householdId', 'horizon', 'idempotencyKey']), outputSchema: RESULT, sourceRefs: ['capability:meal-planning', 'domain:mealPlanContracts'] }),
  makeContract({ id: 'meal_planning.plan.update', owner: 'meal_planning', purpose: 'Update one draft Meal Plan through optimistic reviewed version authority.', authority: 'reviewed', effect: 'write', reversible: true, providers: ['device', 'server'], inputSchema: targetSchema('mealPlanId', { expectedVersion: VERSION, horizon: MEAL_PLAN_HORIZON }, ['expectedVersion', 'horizon']), outputSchema: RESULT, sourceRefs: ['domain:mealPlanContracts'] }),
  makeContract({ id: 'meal_planning.candidate.add', owner: 'meal_planning', purpose: 'Add one reviewed Recipe snapshot or meal note candidate to a draft Meal Plan.', authority: 'reviewed', effect: 'write', reversible: true, providers: ['device', 'server'], inputSchema: targetSchema('mealPlanId', { expectedVersion: VERSION, candidate: MEAL_CANDIDATE_INPUT }, ['expectedVersion', 'candidate']), outputSchema: RESULT, sourceRefs: ['domain:mealPlanContracts'] }),
  ...
  makeContract({ id: 'meal_planning.plan.finalize', owner: 'meal_planning', purpose: 'Finalize one reviewed Meal Plan version under organizer authority.', authority: 'reviewed', effect: 'write', reversible: true, providers: ['device', 'server'], inputSchema: targetSchema('mealPlanId', { expectedVersion: VERSION, occasions: { type: 'array', minItems: 1, maxItems: 60, items: MEAL_PLAN_OCCASION_INPUT }, organizerNote: { type: ['string', 'null'], maxLength: 2_000 }, idempotencyKey: STRING_ID }, ['expectedVersion', 'occasions', 'organizerNote', 'idempotencyKey']), outputSchema: RESULT, sourceRefs: ['domain:mealPlanContracts'] }),
  ...
  makeContract({ id: 'meal_planning.candidates.prepare', owner: 'meal_planning', purpose: 'Prepare up to three budget, stock, gap, cadence, and household-aware Meal candidates without mutating a plan.', authority: 'direct', effect: 'read', reversible: true, providers: ['device','server'], inputSchema: objectSchema({ horizon: MEAL_PLAN_HORIZON, constraints: MEAL_CANDIDATE_CONSTRAINTS },['horizon']), outputSchema: RESULT, sourceRefs:['domain:mealCandidatePreparation'] }),
  ...
  makeContract({ id: 'food_stock.observe', owner: 'groceries', purpose: 'Prepare or record one reviewed owner-only stock observation without promoting receipt evidence to confirmed stock.', authority:'reviewed', effect:'write', reversible:true, providers:['device','server'], inputSchema:objectSchema({observation:FOOD_STOCK_OBSERVATION_INPUT,expectedObservationId:{type:['string','null']},idempotencyKey:STRING_ID},['observation','expectedObservationId','idempotencyKey']), outputSchema:RESULT, sourceRefs:['domain:foodStockContracts'] }),
  makeContract({ id: 'food_stock.deplete', owner: 'groceries', purpose: 'Record reviewed depletion evidence for one stock concept without inferring consumption or food safety from age.', authority:'reviewed', effect:'write', reversible:true, providers:['device','server'], inputSchema:objectSchema({concept:STRING_ID,expectedObservationId:{type:['string','null']},observedAt:{type:'string',format:'date-time'},idempotencyKey:STRING_ID},['concept','expectedObservationId','observedAt','idempotencyKey']), outputSchema:RESULT, sourceRefs:['domain:foodStockContracts'] }),
