...
const OPAQUE_VERSION = { type: 'string', minLength: 1, maxLength: 200 } as const;
...
export const CONTROL_PARITY_OPERATION_CONTRACTS = [
  ...
  write({ id: 'money.transaction.meaning.update', owner: 'money', purpose: 'Correct the explicit meaning of one transaction without changing its plan treatment.', providers: ['device', 'server'], consequence: 'consequential', reversible: true, confirmation: 'explicit', completionMode: 'reviewed_proposal', inputSchema: objectSchema({ transactionId: STRING_ID, expectedUpdatedAt: UPDATED_AT, meaning: { type: 'string', enum: ['income', 'transfer', 'not_counted', 'category_credit'] }, categoryId: STRING_ID }, ['transactionId', 'expectedUpdatedAt', 'meaning']), sourceRefs: moneyRefs }),
  write({ id: 'money.transaction.plan_treatment.update', owner: 'money', purpose: 'Correct one transaction planning treatment without changing provider classification.', providers: ['device', 'server'], consequence: 'consequential', reversible: true, confirmation: 'explicit', completionMode: 'reviewed_proposal', inputSchema: timestampedTargetSchema('transactionId', { treatment: { type: 'string', enum: ['protected', 'flexible', 'default'] } }), sourceRefs: moneyRefs }),
  write({ id: 'money.connection.disconnect', owner: 'money', purpose: 'Disconnect one financial connection after reviewing affected accounts and sync behavior.', providers: ['device', 'server'], consequence: 'consequential', reversible: false, confirmation: 'explicit', completionMode: 'reviewed_proposal', inputSchema: timestampedTargetSchema('connectionId'), sourceRefs: moneyRefs }),
  ...
  write({ id: 'money.transfer.review', owner: 'money', purpose: 'Apply one reviewed transfer pairing or correction.', providers: ['device', 'server'], consequence: 'consequential', reversible: true, confirmation: 'explicit', completionMode: 'reviewed_proposal', inputSchema: timestampedTargetSchema('transferId', { decision: { type: 'string', enum: ['confirm_pair', 'unpair'] } }), sourceRefs: moneyRefs }),
  ...
  write({ id: 'chores.definition.update', owner: 'chores', purpose: 'Update one reviewed Chore definition without rewriting completed occurrence receipts.', providers: ['device', 'server'], consequence: 'consequential', reversible: true, confirmation: 'explicit', completionMode: 'reviewed_proposal', inputSchema: timestampedTargetSchema('choreId', { fields: FIELDS, scope: { type: 'string', enum: ['today', 'this_and_future'] } }), sourceRefs: choreRefs }),
  write({ id: 'chores.definition.pause', owner: 'chores', purpose: 'Pause one Chore definition without completing future occurrences.', providers: ['device', 'server'], consequence: 'consequential', reversible: true, confirmation: 'explicit', completionMode: 'reviewed_proposal', inputSchema: timestampedTargetSchema('choreId'), sourceRefs: choreRefs }),
  write({ id: 'chores.definition.delete', owner: 'chores', purpose: 'Delete one Chore definition while preserving historical receipts.', providers: ['device', 'server'], consequence: 'consequential', reversible: true, confirmation: 'explicit', completionMode: 'reviewed_proposal', inputSchema: timestampedTargetSchema('choreId'), sourceRefs: choreRefs }),
  write({ id: 'chores.occurrence.claim', owner: 'chores', purpose: 'Claim one exact open Household Chore occurrence for the current child.', providers: ['device', 'server'], consequence: 'low', reversible: true, confirmation: 'explicit', completionMode: 'reviewed_proposal', inputSchema: timestampedTargetSchema('occurrenceId'), sourceRefs: choreRefs }),
  write({ id: 'chores.occurrence.release', owner: 'chores', purpose: 'Return one claimed Household Chore occurrence to the open pool.', providers: ['device', 'server'], consequence: 'low', reversible: true, confirmation: 'explicit', completionMode: 'reviewed_proposal', inputSchema: timestampedTargetSchema('occurrenceId'), sourceRefs: choreRefs }),
  write({ id: 'chores.occurrence.complete', owner: 'chores', purpose: 'Complete or submit one exact Chore occurrence under its evidence and approval policy.', providers: ['device', 'server'], consequence: 'consequential', reversible: true, confirmation: 'explicit', completionMode: 'reviewed_proposal', inputSchema: timestampedTargetSchema('occurrenceId', { evidenceRefIds: STRING_LIST }), sourceRefs: choreRefs }),
  write({ id: 'chores.occurrence.reopen', owner: 'chores', purpose: 'Reopen one exact completed Chore occurrence and reverse its current token credit.', providers: ['device', 'server'], consequence: 'consequential', reversible: true, confirmation: 'explicit', completionMode: 'reviewed_proposal', inputSchema: timestampedTargetSchema('occurrenceId'), sourceRefs: choreRefs }),
  write({ id: 'chores.occurrence.report_earlier', owner: 'chores', purpose: 'Report one or more exact missed Chore occurrences for caregiver review.', providers: ['device', 'server'], consequence: 'consequential', reversible: true, confirmation: 'explicit', completionMode: 'reviewed_proposal', inputSchema: objectSchema({ items: { type: 'array', minItems: 1, maxItems: 31, items: { type: 'object', properties: { occurrenceId: STRING_ID, expectedUpdatedAt: UPDATED_AT }, required: ['occurrenceId', 'expectedUpdatedAt'], additionalProperties: false } } }), sourceRefs: choreRefs }),
  ...
  write({ id: 'chores.review.approve', owner: 'chores', purpose: 'Approve one submitted Chore occurrence as an authorized caregiver.', providers: ['device', 'server'], consequence: 'consequential', reversible: true, confirmation: 'explicit', completionMode: 'reviewed_proposal', inputSchema: timestampedTargetSchema('occurrenceId'), sourceRefs: choreRefs }),
  write({ id: 'chores.review.return', owner: 'chores', purpose: 'Return one submitted Chore occurrence as Needs another pass.', providers: ['device', 'server'], consequence: 'consequential', reversible: true, confirmation: 'explicit', completionMode: 'reviewed_proposal', inputSchema: timestampedTargetSchema('occurrenceId', { note: { type: ['string', 'null'], maxLength: 500 } }), sourceRefs: choreRefs }),
  write({ id: 'chores.review.leave_missed', owner: 'chores', purpose: 'Reject an earlier-day correction and leave that occurrence missed.', providers: ['device', 'server'], consequence: 'consequential', reversible: true, confirmation: 'explicit', completionMode: 'reviewed_proposal', inputSchema: timestampedTargetSchema('occurrenceId'), sourceRefs: choreRefs }),
  ...
  write({ id: 'chores.reward.configure', owner: 'chores', purpose: 'Configure the household digital-reward program and token rate.', providers: ['device', 'server'], consequence: 'consequential', reversible: true, confirmation: 'explicit', completionMode: 'reviewed_proposal', inputSchema: objectSchema({ expectedVersion: OPAQUE_VERSION, enabled: { type: 'boolean' }, centsPerToken: { type: 'integer', minimum: 1, maximum: 100000 } }), sourceRefs: choreRefs }),
  write({ id: 'chores.reward.reserve', owner: 'chores', purpose: 'Reserve available tokens at the current household rate for an outside-app payout.', providers: ['device', 'server'], consequence: 'consequential', reversible: true, confirmation: 'explicit', completionMode: 'reviewed_proposal', inputSchema: objectSchema({ membershipId: STRING_ID, tokenCount: { type: 'integer', minimum: 1 }, expectedVersion: OPAQUE_VERSION }), sourceRefs: choreRefs }),
  write({ id: 'chores.reward.cancel', owner: 'chores', purpose: 'Cancel one unpaid token reservation.', providers: ['device', 'server'], consequence: 'low', reversible: true, confirmation: 'explicit', completionMode: 'reviewed_proposal', inputSchema: timestampedTargetSchema('reservationId'), sourceRefs: choreRefs }),
  write({ id: 'chores.reward.settle', owner: 'chores', purpose: 'Record that one outside-app reward payout was completed without moving money in Kwilt.', providers: ['device', 'server'], consequence: 'consequential', reversible: false, confirmation: 'explicit', completionMode: 'reviewed_proposal', inputSchema: timestampedTargetSchema('reservationId'), sourceRefs: choreRefs }),
