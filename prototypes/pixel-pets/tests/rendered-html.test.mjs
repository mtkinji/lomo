...
test("server-renders the Pixel Pet prototype shell", async () => {
  ...
  assert.match(html, /<title>Pet Engine Study 07 — Kwilt Lab<\/title>/i);
...
test("removes starter infrastructure and exposes the portable engine study", async () => {
  ...
  assert.match(layout, /Pet Engine Study 07 — Kwilt Lab/);
  assert.match(prototype, /Pet Engine Study 07/);
  ...
  assert.match(leafling, /leafling-stage-atlas-v1\.png/);
