...
- [x] PostHog funnel dashboard exists. Provisioned 2026-04-17 via the PostHog MCP: [**Email — Deliverability & Funnel (GA)**](https://us.posthog.com/project/266832/dashboard/1480722) (8 tiles: 4 rate BoldNumbers, daily sends by campaign, daily rates, delivery funnel by campaign, full conversion funnel). Will show non-zero events within 24h of first real send post-deploy; single test send already delivered `email.sent` + `email.delivered`.
- [x] PostHog alerts configured for reputation drift. **2 alerts** created on the reputation-critical rates (bounce > 5%, complaint > 0.3%) — free-tier caps at 2/project. Open-rate and click-rate alerts spec'd in runbook §10 for the next plan upgrade. **All alerts currently `enabled: false`** and must be toggled on once real TestFlight sends land (otherwise they fire on the zero-volume denominator).
...
6. ~~**Wire up the dashboard + alerts** per runbook §10.~~ — Done 2026-04-17. Dashboard `1480722` + 2 reputation alerts (bounce, complaint) provisioned via the PostHog MCP; alerts are `enabled: false` until real send volume appears. See runbook §10 for the full tile / alert inventory and the "enable alert" steps.
...
7. ~~**Migrate Welcome Day 0 + Day 1 Resend Automation templates** to carry `List-Unsubscribe` headers (or move them in-repo).~~ — **Done 2026-04-16** (code path). Day 0 and Day 1 now fire from the in-repo `buildWelcomeDay0Email` / `buildWelcomeDay1Email` templates via `sendEmailViaResend`: full `List-Unsubscribe` headers, HMAC unsubscribe URL in the footer, kill-switch, per-user daily cap, preference gating, and PostHog `distinct_id = user_id` all now apply uniformly to the entire welcome drip. Day 0 ships synchronously from the `email-drip` signup handler (fires from the client on successful sign-in); Day 1 picks up on the next scheduled cron (same window and idempotency guards as Day 3/7, keyed on `kwilt_email_cadence.message_key='welcome_day1'`). Edge function `email-drip` deployed 2026-04-16 on project `sqxwjtorodqjdfnuvprf`. **Operational TODO:** pause the legacy "Kwilt Welcome Drip" Automation in the Resend dashboard so the old Day 1 queue doesn't fire. Pre-GA there are effectively no real users in flight, so no cadence backfill needed — worst case is an internal account receiving one duplicate Day 1 email.
