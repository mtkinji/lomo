...
7. ~~**Migrate Welcome Day 0 + Day 1 Resend Automation templates** to carry `List-Unsubscribe` headers (or move them in-repo).~~ — **Done 2026-04-16** (code path). Day 0 and Day 1 now fire from the in-repo `buildWelcomeDay0Email` / `buildWelcomeDay1Email` templates via `sendEmailViaResend`: full `List-Unsubscribe` headers, HMAC unsubscribe URL in the footer, kill-switch, per-user daily cap, preference gating, and PostHog `distinct_id = user_id` all now apply uniformly to the entire welcome drip. Day 0 ships synchronously from the `email-drip` signup handler (fires from the client on successful sign-in); Day 1 picks up on the next scheduled cron (same window and idempotency guards as Day 3/7, keyed on `kwilt_email_cadence.message_key='welcome_day1'`). Edge function `email-drip` deployed 2026-04-16 on project `sqxwjtorodqjdfnuvprf`. **Operational TODO before you ship this code to users on a real build:** pause the legacy "Kwilt Welcome Drip" Automation in the Resend dashboard to prevent the in-flight cohort from receiving a duplicate Day 1 (Automation's queued Day 1 + our cron's in-repo Day 1). For the small pre-GA cohort currently mid-drip, either accept the one-time duplicate or backfill `kwilt_email_cadence` rows with `message_key='welcome_day1'` for every user whose `auth.users.created_at` is more than 24 hours old.
