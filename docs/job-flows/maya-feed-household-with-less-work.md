...
| 8 | **Gather household input.** “What will everyone actually eat?” | Household members nominate recipes and register enthusiasm, “not for me,” or a contextual hard pass in seconds without turning dinner into a formal vote or shaming non-response. A trusted guest can respond to one Plan without becoming a Household member. | Every eligible member can already add and react in the live shared Plan. A learning release infers recipient-owned unseen state and a normal push after a 30-minute burst of new ideas, suppressing contributors and people who already returned. The in-app dot appears on Recipes and Ideas, and a push opens the live Plan directly; there is no inbox, member picker, or response destination. The separate expiring, revocable guest link remains bounded to one Plan snapshot. | 3 | Deploy and prove the quiet-period worker, default member eligibility, per-recipient suppression, in-app dot clearing, preference behavior, simultaneous Realtime Plan updates, and two-account signed-device push/deep-link behavior; separately prove guest-link expiry, revocation, preview, and organizer receipt in a deployed browser. |
