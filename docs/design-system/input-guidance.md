...
| Message or reply | `ChatComposer` | The Unified Chat composition: a fully rounded filled capsule at rest that becomes a rounded two-row surface on focus or entered text, with measured multiline growth and voice/Send actions inside its toolbar. Label/help/audience stay in their established appropriate region. | Draft/retry, submission, dictation execution, keyboard hosting, and persistence remain host-owned. Capability-only tools such as AI context or attachments appear only when the host implements them. |
| Short post | `Input multiline` in its capability-owned composer composition | One filled outer surface; tools may occupy an internal footer slot. | Post placement, publication, attachments, draft/retry, and keyboard hosting. The reference image does not move Post into every composer. |
...
On phone surfaces, the Unified Chat composer frame uses one canonical optical edge gap on the left, right and bottom everywhere it renders. The resting frame remains a full capsule. The inset should keep the composer close to the phone edge while giving its rounded ends balanced clearance from the lower display corners; literal concentric corner geometry is not the target. Presentation and state may change the composer's content and shape, but not its outer placement; this includes full-screen Chat, contextual drawers, bootstrap/loading, resting, expanded, recording, live and keyboard-open states. The site mapping is `--kw-chat-composer-edge-gap`, currently `28px` for the canonical iPhone 17 Pro frame. The composer host remains the only keyboard/safe-area layout owner.
