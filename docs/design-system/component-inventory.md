...
| `ChatComposer` | `src/ui/ChatComposer.tsx` | Canonical | Native message/reply counterpart to the current Unified Chat workbench composer, approved by Andrew on 2026-09-11. It owns the same compact pill at rest, focused two-row expansion, filled material, measured growth, internal voice/Send actions, visible recording/transcription/retry states, and accessible action labels. Hosts retain draft, submission, dictation execution, keyboard-layout, and persistence ownership. Home conversation replies are the first native adopter; Unified Chat remains the visual and behavior reference in its web workbench. |
