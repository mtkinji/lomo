...
| Bottom dock frames | `src/ui/ActionDock.tsx`, `src/ui/FullWidthActionDock.tsx`, `src/ui/layout/BottomDrawerFooter.tsx`, and `BottomDrawer.bottomAccessory` | Canonical | Phone-floating actions and full-width drawer actions share semantic optical nesting without sharing action anatomy. `FullWidthActionDock` is the default host for one persistent full-width page button; the UI layer owns safe-area, inset, and scroll-clearance geometry while feature code supplies the canonical `Button`. Approved by Andrew on 2026-08-17 and extended to full-width page buttons on 2026-08-20. |
