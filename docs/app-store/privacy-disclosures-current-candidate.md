# App Store Privacy Disclosures — Current Kwilt submission candidate
...
Use this packet for App Store Connect only after the September 2, 2026 Privacy Policy is live. Reconfirm it against the exact archive selected for submission and the deployed service configuration. App Store Connect answers describe the submitted binary, including optional features and integrated third-party code. They are separate from `PrivacyInfo.xcprivacy`, which declares app-collected data and required-reason APIs while incorporated SDK manifests declare their own behavior.
...
- Privacy Policy URL: `https://www.kwilt.app/privacy`
- Privacy Choices URL: `https://www.kwilt.app/privacy#privacy-choices`
...
- [x] September 2 policies are deployed and all `kwilt.app`, `www.kwilt.app`, and `go.kwilt.app` legal URLs resolve to them (verified September 3, 2026; Vercel production deployment `dpl_CZKzm5kkCvajtP42Tk2aie6p77zp`).
- [x] App Store Connect published all 22 selected data types above on September 3, 2026. Every type's detail says linked to the user's identity, and the product-page preview contains no Data Used to Track You section.
- [x] The Privacy Policy URL is published in App Store Connect. The Privacy Choices URL is saved as an edit that App Store Connect says will be released with the next app version.
...
## Archive privacy-report reconciliation

On September 3, 2026, Xcode 26.6 generated an aggregated privacy report from an unsigned local Release archive. The report contained all 22 app declarations in this packet and these incorporated SDK declarations:

- LinkKit: User ID, linked, not tracking, User Identifier purpose.
- RevenueCat: Purchase History, not linked, not tracking, App Functionality purpose.
- react-native-maps: Precise Location, not linked, not tracking, App Functionality purpose.

No component declared tracking. The archive also contained privacy manifests for Expo, React Native, Async Storage, WebRTC, LinkKit, RevenueCat, and maps dependencies, including the required-reason API declarations expected from the dependency graph.

This was diagnostic evidence only: the local archive identified itself as `1.0.118 (118)`, was unsigned, and was built from a working tree rather than the exact source selected for App Store submission. Generate and inspect the report again from the final signed archive.
