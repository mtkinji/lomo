...
The substrate is now defined as a shared durable Kwilt Agent Runtime with distributed providers, not merely a list of similarly named channel operations. Phone Agent should use the same run, discovery, policy, proposal, and receipt contracts as in-app Chat for ordinary reasoning and server-capable tools. SMS/voice compliance stays deterministic at the channel boundary, and any device-only or in-app confirmation step persists as pending work instead of being reported as complete.

The app process is not the Phone Agent backend. MCP is an external projection of server-capable tools, not an internal hop required by mobile or telephony.

The current implementation slice routes ordinary verified SMS into a durable, idempotent channel-job queue and processes it through the same bounded canonical run coordinator used by Chat. STOP, START, HELP, done, snooze, pause, not relevant, and change-time commands remain deterministic at the webhook boundary. For ordinary cross-domain questions, the bounded loop can call multiple owner-scoped capability reads and synthesize them while preserving capability-owned priority and status. The worker rechecks link state, checkpoints model output before delivery, resumes multipart delivery by Twilio SID, and applies low-risk Activity capture through an atomic proposal/operation/receipt RPC. It can also stage Profile display-field update; Arc create/update/delete; Goal create/update/delete; Activity update/complete/delete, stable step create/update/complete/delete/reorder, recurrence, reminder, and focus-today; Chapter private-note updates; and Plan schedule/reschedule/remove plus grouped chunks for review through the same durable proposal records and native mobile review paths. The local coaching Profile stays authoritative: mobile sync publishes only profile id, display name, age range, and optimistic version to an owner-scoped server projection; Phone reads that projection and stages edits for native Profile review rather than creating a second profile store. The prior relationship extractor now has a primary typed tool path shared with mobile Chat: the model can interpret varied language into one explicit named-person payload, while strict validation and a separate Phone permission bound what can persist. Remembering reuses the existing People/Memory/Event/Cadence tables and atomically records trust evidence and an action log without requiring an incidental Activity. Correction and forgetting first read the exact record id and optimistic version, then atomically change only that record and emit the same trust evidence. Exact correction and individual-record forgetting receipts can be undone later from mobile Chat through the same authenticated service transaction. Remembering is compensatable through exact-record forgetting but has no one-tap receipt Undo yet; whole-person forgetting is excluded until every dependent record can be reviewed and restored safely. The regex extractor remains only as a compatibility fallback and is suppressed when the typed remember tool succeeds. A chunk group enters persistence through one atomic service RPC; native Plan still applies each provider event sequentially with its own receipt and undo. Owner and optimistic-version checks ground changes to existing records; Activity-to-Goal reassignment validates the parent owner, step changes address stable ids, and Plan placement validates the configured active provider calendar. Reminder approval updates the existing Activity and leaves device notification scheduling to the native notification service and device settings. Focus-today remains the existing soft Plan signal. Focus, Activity location, attachment selection, Activity sharing, Goal sharing, and Plan preference management stage owner-scoped native handoffs; the Phone turn cannot claim their device-side effects completed. The coordinator deterministically reports staged proposals and pending native actions as not yet applied, even if model-authored text claims otherwise. This remains pre-release until the migrations, functions, scheduler, signed Twilio flow, and physical-device timeline are proven.

Phone does not get separate AI-shaped versions of these experiences. The owning capability defines prioritization, proposal content, approval, receipt, recovery, and native return; Phone and Chat add conversational entry and explanation around those same records and components.

Implementation checkpoint (2026-07-23): Phone Plan read/recommend calls the same `@kwilt/plan-core` priority and eligibility kernel as native Plan. The linked phone timezone grounds relative requests such as “tomorrow,” and recommendation output preserves the owned ranking while explicitly withholding calendar-fit claims when availability is not loaded. When the user supplies an explicit placement, Phone can stage schedule/reschedule/remove or a 2–10 chunk group through the existing native Plan review, provider apply, receipt, recovery, and undo path. Group creation is atomic; provider application remains sequential and event-specific. The Phone turn never reports any calendar write as complete. This remains local code/test evidence until the migrations and functions are deployed and signed Phone continuity is exercised.
