## feature brief — Kwilt Desktop App (v1)
...
Gate heavy AI operator usage behind Pro, aligned with mobile's existing quota model from `docs/feature-briefs/ai-proxy-and-quotas.md`. The free tier should include enough AI planning to show value, while core workspace, Views, quick capture, and basic Focus remain useful without AI.
...
This feature brief is the strategic roadmap. Each milestone below maps to one (or more) small, executable `.plan.md` files under `~/.cursor/plans/`. Create each in order; earlier milestones unblock later ones. Don't mix milestones in a single build plan.
...
- No "decide later" or "if not tractable" language — those decisions happen in this feature brief or in chat before Build runs.
- Always link back to the relevant section of this feature brief for *why*.
