...
**Chosen experience:** Home Create offers Photo and Write; both enter one editor. Photo launches library selection and then leads with the selected image; Write focuses the caption/body with Add photos available. Capability Share enters with the exact approved attachment already visible. User can reorder/remove photos, edit alt descriptions and caption, remove an attachment, and choose audience without changing source content.

Header: Close and “Share a moment.” One bottom primary action: “Post to [household name],” “Post to 3 people,” or “Post to approved followers.” The always-visible audience chip opens a picker for the existing three audience modes. With the keyboard open, keep that single action above it. No second top-right Post. For first use, preselect the sole eligible household if one exists; otherwise require an audience choice. Existing draft/offer audience wins over inferred defaults. Never derive it from the current feed filter.
...
**Acceptance:** 100-item mixed feed; new post while reading older pages; deleted anchor; reply then Back; photo index then Back; source action then Back; offline toggle; stale response; account switch and access removal. Correct position and authority take precedence over animation polish.
