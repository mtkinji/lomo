## feature brief — Growth Flywheel (Evangelism + Social Accountability + Shared Goals)
...
This feature brief intentionally defines phases so we can ship “launch-safe” pieces first and grow into richer shared goals later.
...
- **`jtbd-invite-the-right-people-in`** — the entire posture of this feature brief (per-Goal sharing granularity, explicit invite friction, privacy as first-class) is the supply-side realization of "help me invite the right people into my becoming without losing privacy." The Phase ordering (share message → link → shared Goal) deliberately mirrors a *deepening of trust*, not a growth-loop funnel.
- **`jtbd-trust-this-app-with-my-life`** — privacy expectations and explicit-about-what-is-shared listed under Risks are not ancillary; they're load-bearing for the meta-JTBD around trust. Any growth-loop optimization that pushes against this trade-off costs trust.

**Note**: this feature brief's framing is somewhat *supply-side* (growth flywheel language). The corresponding *demand-side* JTBD `jtbd-invite-the-right-people-in` is stricter — it explicitly rejects "share to unlock" dark patterns and feed-style social. When trade-offs arise during Phase 1+ implementation, the JTBD wins.
