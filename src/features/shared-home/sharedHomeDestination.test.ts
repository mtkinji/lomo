...
describe("resolveSharedHomeDestination", () => {
  ...
  it("opens a game turn in its exact remote room", () => {
    expect(
      resolveSharedHomeDestination({ kind: "game_room", sessionId: "room-1" }),
    ).toEqual([
      "Games",
  ...
  it("opens shared Goal content at the authoritative Goal", () => {
    expect(
      resolveSharedHomeDestination({ kind: "goal", goalId: "goal-1" }),
    ).toEqual([
      "MainTabs",
      {
        screen: "GoalsTab",
  ...
  it("opens a meal choice invitation at an authoritative participant refetch", () => {
    expect(
      resolveSharedHomeDestination({ kind: "meal_choice", roundId: "round-1" }),
    ).toEqual([
      "Food",
      { screen: "MealChoiceResponse", params: { roundId: "round-1" } },
    ]);
  });
});
