...
describe("Shared Home presentation", () => {
  ...
  it("rejects unknown event and destination kinds", () => {
    expect(
      parseSharedHomeRow(row({ event_kind: "marketing" }), now),
    ).toBeNull();
    expect(
      parseSharedHomeRow(
        row({ destination: { kind: "arbitrary_route", name: "Settings" } }),
        now,
      ),
    ).toBeNull();
  });
  ...
  it("derives expiry without mutating source authority", () => {
    const delivery = parseSharedHomeRow(
      row({ expires_at: "2026-08-05T11:00:00.000Z" }),
      now,
    )!;
  ...
  it("redacts unavailable presentation", () => {
    expect(
      parseSharedHomeRow(row({ state: "unavailable" }), now),
    ).toMatchObject({
  ...
  it("groups pending separately from shared content and sorts newest first", () => {
    const pendingOlder = parseSharedHomeRow(
      row({ id: "pending-old", created_at: "2026-08-05T09:00:00.000Z" }),
      now,
    )!;
    const pendingNewer = parseSharedHomeRow(
      row({ id: "pending-new", created_at: "2026-08-05T11:00:00.000Z" }),
      now,
    )!;
    const settled = parseSharedHomeRow(
      row({
        id: "settled",
        state: "settled",
    ...
    const available = parseSharedHomeRow(
      row({
        id: "checkin",
        event_kind: "goal_checkin",
        source_entity_type: "goal_checkin",
    ...
    const groups = groupSharedHomeDeliveries(
      [pendingOlder, settled, available, pendingNewer],
      now,
    );
    expect(groups.needsYou.map((item) => item.id)).toEqual([
      "pending-new",
      "pending-old",
    ]);
    expect(groups.sharedWithYou.map((item) => item.id)).toEqual([
      "checkin",
      "settled",
    ]);
  });
