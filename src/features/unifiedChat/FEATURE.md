...
The active learning release adds a durable operational control plane around the bounded `gpt-5.6-luna` judgment pass. A versioned Turn Contract carries job, desired outcome, constraints, capability scope, operation identity, target resolution, and correction/retry referents through context selection and execution. Capability-owned validation, permission, confirmation, proposals, mutation, native handoff, receipts, recovery, and undo remain authoritative, and visible action status is projected from those facts rather than model prose.
...
The canonical interpretation, authority, evidence, explanation, and evaluation rules are in `docs/product/unified-chat-behavior-contract.md`. The structured judgment now carries explicit action authority, evidence scope, and response contract; ordinary domain phrase matches may seed fallback routing but no longer lock out a coherent semantic plan. The original Money dogfood transcript is retained in a varied behavioral evaluation family rather than production routing logic.
...
- The expanded judgment contract is source- and test-proven, and `ai-chat` version 65 now deploys the bounded `agent_judgment` proxy route with JWT verification. An authenticated development-Simulator text probe routed “What should I do tomorrow” into Plan priority and kept its recommendations analysis-only. The held-out live Luna operation and behavior evaluations have not been run, and the explicit-date, multi-tool, voice, broad-evidence, signed-build, and physical-device matrix remains open. Earlier dogfood failures remain contradictory runtime evidence outside this narrow proof until those gates pass.
- Text, native voice, explicit Kwilt-object context, and up to three durable attachment provenance records are source- and test-complete. Finalized Conversation utterances now use a stable provider-item request id and the same server-owned durable run path as typed text; interim deltas remain UI-only, and bounded voice provenance persists in channel context. Signed-device voice continuity and latency proof remain open. Text stays locally decoded; bounded PNG, JPEG, WEBP, and PDF bytes are inspected transiently through the authenticated Kwilt AI proxy, while only the per-turn observations and explicit limits persist. Draft removal prevents send-time use, failed inspection blocks send, and assistant-authored document/checklist/table/code artifacts persist as editable drafts rather than capability receipts.
...
Apply the remaining new migrations and functions only after explicit authorization, then prove the standing typed/voice matrix on a signed simulator and physical device. The authenticated `ai-chat` `agent_judgment` route is deployed separately as version 65. The highest-risk runtime checks are Plan recommendation/apply, stale and duplicate proposal handling, Goal check-in audience review, pending device-action resume, background/restore, and a phone-started run continuing in mobile Chat without duplicate work.
