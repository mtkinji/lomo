...
The 2026-08-01 learning release adds one bounded `gpt-5.6-luna` judgment pass before capability context loading. It identifies the user's practical job, desired outcome, explicit constraints, smallest relevant tool set, and direct/single/multi-tool execution mode. Deterministic safety locks remain prior constraints, the older semantic route is a recoverable fallback, and capability-owned validation, permission, confirmation, proposals, mutation, native handoff, receipts, recovery, and undo remain authoritative.
