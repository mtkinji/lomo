import {
  applyApprovedProfileProposal,
  prepareApprovedProfileProposal,
...
export async function executeProfileProposalDecision({ proposal, action, repository, store, now = () => new Date().toISOString() }: {
  proposal: ProfileProposal;
  action: DecideUnifiedChatProposalInput['action'];
  repository: Repository;
  store: ProfileStoreBoundary;
  now?: () => string;
}): Promise<void> {
  ...
  try {
    const appliedAt = now();
    const reservation = prepareApprovedProfileProposal({ proposal: approved, store, appliedAt });
    const reserved = await repository.persistMutationReceipt({
      capabilityId: 'profile', threadId: proposal.threadId, proposalId: proposal.id,
      operationId: proposal.operation.id, idempotencyKey: proposal.operation.idempotencyKey,
      status: 'reserved', resultingObjectType: 'profile', resultingObjectId: reservation.resultingObjectId,
    ...
    const result = applyApprovedProfileProposal({ proposal: approved, store, now: () => appliedAt });
    await repository.finalizeMutationReceipt(reserved.id, {
      capabilityId: 'profile', resultingObjectType: 'profile', resultingObjectId: result.resultingObjectId,
