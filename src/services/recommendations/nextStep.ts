...
export function getSuggestedNextStep(params: {
  ...
  const actionable = activities.filter(isActionable);
  if (actionable.length === 0) {
    return { kind: 'setup', reason: 'no_activities' };
  }

  const scored = actionable.map((activity) => {
    ...
    const updatedScore = Number.isNaN(updatedAt) ? 0 : Math.min(1, (Date.now() - updatedAt) / (7 * 24 * 60 * 60 * 1000));
    // Higher is better.
