...
describe('Grocery Savings screen', () => {
  ...
  it('opens from capability navigation without a specific grocery list', async () => {
    (createGroceryRepository as jest.Mock).mockReturnValue({ list: jest.fn().mockResolvedValue([]) });
    const screen = render(
      <GrocerySavingsScreen
    ...
    await waitFor(() => expect(screen.getByText('No verified offers yet')).toBeTruthy());
    expect(createGrocerySavingsRepository).not.toHaveBeenCalled();
  });
});
