...
describe('Grocery repository', () => {
  it('compiles through the server boundary and mutates only through revisioned RPCs', async () => {
    ...
    await repository.compile('plan-1',3); await repository.compile('plan-1',4,{fromListId:'list-1',expectedRevision:5}); await repository.setItemState('item-1',2,'already_have'); await repository.addItem('list-1',3,'Dish soap'); await repository.markReviewed('list-1',4); await repository.handoff('list-1',5,'instacart'); await repository.markHandoffOpened('handoff-1');
    ...
    expect(invoke).toHaveBeenNthCalledWith(3,'grocery-handoff',{body:{groceryListId:'list-1',expectedRevision:5,provider:'instacart'}});
    expect(invoke).toHaveBeenNthCalledWith(4,'grocery-handoff',{body:{action:'opened',handoffId:'handoff-1'}});
  });
