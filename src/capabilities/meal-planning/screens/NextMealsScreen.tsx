...
export function finalizedOccasionSummaries(plan: MealPlanProjection): Array<{ id: string; title: string; date: string | null; dishes: Array<{ id: string; label: string }> }> {
...
export function NextMealsScreen({ navigation }: Props) {
  ...
  return (
    <AppShell>
      <PageHeader title="Meal Plan" onPressBack={() => navigation.goBack()} rightElement={<Button size="sm" onPress={() => navigation.navigate('MealPlanEditor', {})}>{plan ? 'New cycle' : 'Start'}</Button>} />
        ...
        {plan ? (
          <View style={styles.planCard}>
            ...
            <View style={styles.meals}>{plan.state === 'finalized'
              ? finalizedOccasionSummaries(plan).map((occasion) => <View key={occasion.id} style={styles.occasion}><Text variant="label">{occasion.title}{occasion.date ? ` · ${occasion.date}` : ''}</Text>{occasion.dishes.map((dish) => <Text key={dish.id} tone="secondary">{dish.label}</Text>)}</View>)
...
const styles = StyleSheet.create({
  ...
  planCard: { padding: spacing.lg, gap: spacing.md, borderWidth: 1, borderColor: colors.cardBorder, backgroundColor: colors.card, borderRadius: 18 }, meals: { gap: spacing.sm }, occasion: { gap: spacing.xs }, actions: { gap: spacing.sm },
});
