...
import { buildMealPlanningRecipeInventory, orderMealPlanningRecipeInventory } from './mealPlanningRecipeInventory';
...
describe('Meal Planning recipe inventory', () => {
  it('includes the bundled catalog when the household has not saved a private recipe', () => {
    ...
    expect(inventory.every((projection) => projection.recipe.provenance.method === 'catalog')).toBe(true);
  });
  ...
  it('moves the prepared candidates to the top in their ranked order', () => {
    ...
    expect(ordered.slice(0, 2).map((projection) => projection.recipe.id)).toEqual(rankedIds);
