...
describe('shared household Plan projection', () => {
  it('preserves server lifecycle ordering and actor-scoped authority', () => {
    const cart = parseSharedMealCartProjection({
      ...
      candidates: [
        { id: 'candidate-2', kind: 'meal_note', title: 'Second', recipeSnapshot: null, position: 1, createdAt: '2026-08-11T02:00:00Z', lifecycle: 'sent', sentAt: '2026-08-11T03:00:00Z', missingItemCount: 1, voteCount: 2, downvoteCount: 0, reactionCounts: { thumbs_up: 1, heart: 1, yum: 0, excited: 0, fire: 0, downvote: 0 }, contributor: { personId: 'person-owner', displayName: 'Maya' }, supporters: [{ reactionId: 'reaction-owner', personId: 'person-owner', displayName: 'Maya', reaction: 'heart' }, { personId: 'person-2', displayName: 'Sam', reaction: 'thumbs_up' }], viewerReaction: 'heart', canReact: true, canRemove: true, canMarkMade: true },
    ...
    expect(cart?.candidates[0]?.supporters[0]?.reactionId).toBe('reaction-owner');
