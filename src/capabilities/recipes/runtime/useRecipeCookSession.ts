...
export function useRecipeCookSession(recipe: RecipeProjection, servings: number) {
  ...
  useEffect(() => { let alive = true; void (async () => { const cached = userId ? await repository.readActive(userId) : null; if (alive && cached?.recipeVersionId === recipe.currentVersion.id && ['active','paused'].includes(cached.status)) setSession(cached); if (alive) setRestoring(false); })(); return () => { alive = false; }; }, [recipe.currentVersion.id, repository, userId]);
  useEffect(() => {
  ...
  const startTimer = useCallback(async (suggestion: CookCue['timerSuggestions'][number]) => {
    ...
    if (permission.status === 'granted') notificationId = await Notifications.scheduleNotificationAsync({ content: { title: `${suggestion.label} timer`, body: `${recipe.currentVersion.title} is ready for the next step.`, data: { capability: 'food', recipeId: recipe.recipe.id, cookSessionId: session.id } }, trigger: { type: Notifications.SchedulableTriggerInputTypes.DATE, date: new Date(Date.now() + suggestion.durationSeconds * 1000) } });
