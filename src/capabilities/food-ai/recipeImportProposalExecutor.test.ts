...
describe('Recipe import proposal executor', () => {
  it('sends only reviewed data to the approval RPC and is retry-keyed', async () => {
    ...
    await executor.approve({ draftId: 'draft-1', expectedDraftVersion: 2, idempotencyKey: 'approval-1', reviewedData: { title: 'Soup' } });
    expect(rpc).toHaveBeenCalledWith('approve_kwilt_recipe_import_conversational', {
      p_draft_id: 'draft-1', p_expected_draft_version: 2, p_idempotency_key: 'approval-1',
